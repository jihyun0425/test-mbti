class PersonalTest {
    constructor(target) {
        ...
        this.questions = {
            ...
            SN: [{ question: '멍 때릴 때 내가 하는 생각은?', answer: { a: '여러가지 잡생각', b: '아무 생각이 없다' } }],
            TF: [{ question: '친구가 우울해서 미용실에 갔다 왔다고 한다', answer: { a: '왜 우울해? 무슨 일 있었어?', b: '머리 뭐 했어?' } }],
            JP: [{ question: '내 카카오톡 상태는?', answer: { a: '읽지 않은 알람이 거의 없다', b: '읽지 않은 알람이 매우 많다' } }],
        }; // 질문 모음
        this.results = []; // 사용자가 선택한 답모음
        this.resultInfors = {
            ISTJ: {title:"청렴결백 논리주의자",
